...
import { MintLayout, SnipeType } from "./types";
...
async function getTokenAmountAfterBuy(mint: PublicKey): Promise<number> {
  ...
  logger.info(
    { mint: mint.toString() },
    "Checking token accounts after buy..."
  );
  ...
  setInterval(async () => {
    for await (const token of buyList) {
      ...
      if (!!info) {
        const poolState = LIQUIDITY_STATE_LAYOUT_V4.decode(info.data);
        const price =
          poolState.swapQuoteOutAmount / poolState.swapQuoteInAmount;
